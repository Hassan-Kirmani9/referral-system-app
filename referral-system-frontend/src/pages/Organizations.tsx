...
import { get, post, _delete, put } from '../services/api';
import type { Organization, ApiResponse, OrganizationType, OrganizationRole } from '../types';
...
export const Organizations = () => {
  ...
  const [deleteDialog, setDeleteDialog] = useState<{ show: boolean; id: string; name: string }>({
    show: false,
    id: '',
    name: '',
  });
  ...
  const fetchOrganizations = async () => {
    try {
      ...
      setToast({ type: 'error', message: error.error || 'Failed to fetch organizations' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.type || !formData.role || !formData.email || !formData.phone) {
    ...
    try {
      setLoading(true);
      const orgResponse = await post<ApiResponse<Organization>>('/organizations', {
        name: formData.name,
        type: formData.type,
        role: formData.role,
        contact: {
      ...
      setToast({ type: 'success', message: 'Organization created successfully!' });
      fetchOrganizations();
      
      setFormData({ name: '', type: '', role: '', email: '', phone: '' });
      setCoverageAreas([{ state: '', county: '', city: '', zipCode: '' }]);
    } catch (error: any) {
      setToast({ type: 'error', message: error.error || 'Failed to create organization' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      setToast({ type: 'error', message: error.error || 'Failed to delete organization' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const addCoverageArea = () => {
    setCoverageAreas([...coverageAreas, { state: '', county: '', city: '', zipCode: '' }]);
  };
  ...
  return (
    <div className="space-y-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-4">
              {['SENDER', 'RECEIVER', 'BOTH'].map((role) => (
                <label key={role} className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-sm">{role === 'BOTH' ? 'Can Send & Receive' : `Can ${role.toLowerCase()} Only`}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-3">
              <label className="block text-sm font-medium text-gray-700">Coverage Areas</label>
              <button
                type="button"
            ...
            {coverageAreas.map((area, index) => (
              <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-3 p-3 border rounded-md">
                ...
                <input
                  type="text"
                  value={area.county}
                  onChange={(e) => updateCoverageArea(index, 'county', e.target.value)}
                ...
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={area.zipCode}
                    onChange={(e) => updateCoverageArea(index, 'zipCode', e.target.value)}
      ...
      {/* Organizations List */}
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-xl font-bold mb-4">All Organizations ({organizations.length})</h2>
            ...
            <tbody className="divide-y">
              {organizations.map((org) => (
                <tr key={org.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <button
                      onClick={() => setDeleteDialog({ show: true, id: org.id, name: org.name })}
                      className="text-red-500 hover:text-red-700"
                    >
                      <Trash2 size={18} />
                    </button>
